...
\begin{equation}
\label{eq:vecsum}
\vec{A} = \Sigma W_x\vec{B_x}
\end{equation}

where:
\\
$\vec{A}$ = the final acceleration vector
\\
$B_x$ = the behavior $x$, for example $B_0$ could be cohesion behavior, $B_1$ alignment behavior etc.
\\
...
D_{turn} = (R_{goal} - R{current})* \frac{180}{\pi}
\end{equation}
...
\\
$D_{turn}$ = the angle the robot needs to turn to face the correct direction, measured in degrees
\\
...
Then by using the arctangent of $V_x$ and $V_y$, it finds out which angle it needs to face to be able to face. When the robot has calculated which direction it wants to go, it calculates $D_{turn}$ by using the formula found in equation \ref{eq:angletoturn}. $D_{turn}$ is the amount of degrees the robot needs to turn to face the right direction. 
