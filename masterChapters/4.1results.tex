...
All of the results for each scenario was then added together and divided by the number of runs used on that experiment (five runs on simulator and ten runs on the physical experiment) to make the plot show the average of all the runs.
...
% All results for the physical experiment might be a little imprecise, but they should give an idea how the robots are behaving. Meaning that, the graphs can not be used to tell exactly how fast they are moving or how far the robots are from each other. It can be used to tell that the robots might move faster at certain part of experiment, and that they are approximately 
...
The camera used in this experiment is a web camera. To be able to get clear stable video feed images from the web camera, the settings for the camera had to be manually set up.
...
%\end{figure}
The first scenario was designed to check if the robots were able to flock together, the robots were placed on each of the sandbox's corner, as far from each other as possible.
...
The robots do crash into each other sometimes, that is they do push each other around when they are trying to occupy the same space as the other robots. This might happen if both robots are measuring the distances around them, and either some disturbances make the distance measured inaccurate or the other robots move into the other ones' path between the time they are doing the distance measuring.
